    ...
    if (this.baseFormData.physicalAsset) {
      ...
      if (res?.data?.defaultAccessories) {
        ...
        if (savedAccessories.length > 0) {
          const existingIdName = savedAccessories.map(acc => acc.name);
          const filteredApiResponse = apiResponse.filter(
            acc => !existingIdName.includes(acc.valueText)
          );
          const updatedSaved = savedAccessories.map(saved => {
            const match = apiResponse.find(apiAcc => apiAcc.valueText === saved.name);
            return match
              ? { ...saved, type: match.type, valueText: match.valueText, customflowID: match.customFlowID ?? saved.customflowID }
              : saved;
          });
          // console.log("updatedSaved",updatedSaved);

          this.finalAccessories = [...updatedSaved, ...filteredApiResponse];
        }
        else {
          this.finalAccessories = apiResponse;
        }
        // console.log(this.finalAccessories);
        ...
        if (apiResponse.length == 10) {
          ...
          this.otherAccessories = [...this.otherAccessories];
        }
        else {
          //   this.defaultAccessories = this.finalAccessories.slice(0, 9).map(acc => ({
          //                         ...acc,
          //                         isDefault: true
          //                       }));
          // this.defaultLength =this.defaultAccessories.length;

          // this.otherAccessories = apiResponse.slice(9).map(acc => ({
          //   ...acc,
          //   isDefault: false
          // }));
          // this.otherAccessories = [...this.otherAccessories, other];
          ...
          this.defaultLength = this.defaultAccessories.length;
          this.otherAccessories = apiResponse
        ...
        // if (this.otherAccessories.length) {
        //   this.addAccessory();
        // }
