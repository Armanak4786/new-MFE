...
export class TrustService extends BaseDealerService implements OnInit {
  ...
  componentValidity = {
    "Trust Details": [
      { TrustDetailComponent: false },
      { TrustContactDetailsComponent: false },
      { TrustEmailContactDetailsComponent: false }
    ],
    "Address Details": [
      { TrustPhysicalAddressComponent: false },
      { TrustPostalAddressComponent: false },
    ...
    "Finance Accounts": [
      ...
      { TrustAssetsComponent: false },
      { TrustLiabilitiesComponent: false },
    ],
    "Trustee Details": [
...
sortOptionsWithSelectedOnTop(list: any[], selectedValue: string | null): any[] {
  ...
  console.log('Sorting trust options with selected value:', selectedValue);
...
sortNatureTrustOptionsWithSelectedOnTop(list: any[], selectedValue: string | null): any[] {
  if (!list || list.length === 0) return [];
  
  console.log('Sorting nature of trust options with selected value:', selectedValue);
  
  // Extract description text helper (ignoring numeric codes)
  ...
  if (!selectedValue) {
    // Sort alphabetically by description text (ignoring numeric codes)
    return [...list].sort((a, b) => {
      const textA = extractDescriptionText(a.label);
      const textB = extractDescriptionText(b.label);
      return textA.localeCompare(textB, undefined, { sensitivity: 'base' });
    });
  }

  // Find the selected option
  const selectedOption = list.find(opt => opt.value === selectedValue);
  
  if (!selectedOption) {
    // If selected value not found, sort alphabetically by description text
    return [...list].sort((a, b) => {
      const textA = extractDescriptionText(a.label);
      const textB = extractDescriptionText(b.label);
      return textA.localeCompare(textB, undefined, { sensitivity: 'base' });
  ...
  // Filter out selected option and sort the rest alphabetically by description text
  const remainingOptions = list
  ...
  // Return selected option first, followed by the rest
